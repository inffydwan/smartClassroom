...
void LCD_Init(void)
{
    ...
    /* 配置第一和第二层的透明度,最小值为0(完全透明），最大值为255*/
...
/**
  * @brief  选择LCD层
  * @param  LayerIndex: 前景层(1)或者背景层(0)
  * @retval 无
  */
...
static void DrawChar(uint16_t Xpos, uint16_t Ypos, const uint8_t *c)
{
  ...
  offset =  8 *((width + 7)/8) -  width ;//计算字符的每一行像素的偏移值，实际存储大小-字体显示宽度
  ...
  for(i = 0; i < height; i++)//遍历字体高度绘点
  {
    ...
    case 1:
      line =  pchar[0];      //提取字体宽度小于或等于8的字符的像素值
      break;
      
    case 2:
      line =  (pchar[0]<< 8) | pchar[1]; //提取字体宽度大于8小于或等于16的字符的像素值     
      break;
      
    case 3:
    default:
      line =  (pchar[0]<< 16) | (pchar[1]<< 8) | pchar[2]; //提取字体宽度大于16小于或等于24的字符的像素值     
      break;
    } 
...
{ 
	...
	/*GB2312 公式:
	第一个字节为“高字节”，对应 94 个区，每区含有 94 个位
	*/
...
static void LCD_DispChar_CN (uint16_t Xpos, uint16_t Ypos, uint16_t Text)
{
  ...
  height = 	HEIGHT_CN_CHAR;//取字模数据//获取正在使用字体高度
  width  =  WIDTH_CN_CHAR; //获取正在使用字体宽度
  
  offset =  8 *((width + 7)/8) -  width ;//计算字符的每一行像素的偏移值(无效像素点个数），实际存储大小-字体显示宽度
  ...
  for(i = 0; i < height; i++)//遍历字体高度绘点
  {
    ...
    case 1:
      line =  pchar[0];      //提取字体宽度小于等于8的字符的像素值
      break;
      
    case 2:
      line =  (pchar[0]<< 8) | pchar[1]; //提取字体宽度大于8小于等于16的字符的像素值     
      break;
      
    case 3:
    default:
      line =  (pchar[0]<< 16) | (pchar[1]<< 8) | pchar[2]; //提取字体宽度大于16小于等于24的字符的像素值     
      break;
    } 
...
{  
  ...
  while ((refcolumn < LCD_PIXEL_WIDTH) && ((*ptr != 0) & (((refcolumn + DrawProp[ActiveLayer].pFont->Width) & 0xFFFF) >= DrawProp[ActiveLayer].pFont->Width)))
  {
	...
	/*当 2 个大于 127 的字符连在一起时，就表示 1 个汉字
